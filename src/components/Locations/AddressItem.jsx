  ...
  const removeUserLocation = async (locationIdToRemove) => {
    try {
      ...
      const response = await fetch(`http://localhost:8000/users/${user._id}`, {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          locations: user.locations.filter(location => location !== locationIdToRemove)
        }),
      });
  ...
  return (
    <div className={styles.addressItem}>
      ...
      <div className={styles.addressItemControl}>
        <button className={styles.removeUserLocation} type="button" value={_id} onClick={handleRemove}>
